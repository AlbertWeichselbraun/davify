...
def upload(local_fname, lifetime, webdav_file_pattern, file_url_pattern):
    ...
    file_url_dict = get_file_name_dict(local_fname, lifetime,
                                       get_version_suffix())
...
def parse_arguments():
    ...
    parser.add_argument('fname', help='File(s) to davify.', nargs='*',
                        default=None)
    parser.add_argument('--lifetime',
                        help="Suggested file lifetime (default: '1 week'). "
                             ...
                             "deleted.",
                        default='1 week')
    parser.add_argument('--retrieval-url-pattern',
                        help='Pattern to use for the retrieval URL.')
    parser.add_argument('--webdav-file-pattern',
...
if __name__ == '__main__':
    ...
    if not which('xclip'):
        warn("Binary 'xclip' not found. Cannot copy davify URL to clipboard. "
    ...
    if args.setup:
        ...
        if args.lifetime not in VALID_LIFE_TIMES:
            print('Invalid lifetime - please refer to the list below for'
                  'valid choices')
            print('-----------------------------------------------------'
                  '--------------')
        ...
        if not args.fname:
            ...
            with TemporaryDirectory() as tempdirname:
                filename = os_join(tempdirname, args.archive_name
                                   if args.archive_name
                                   else get_archive_name(
                                       args.fname[0])+TAR_FILE_EXT)
